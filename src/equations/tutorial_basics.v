(** * Tutorial Equations : Basic Definitions and Reasoning with Equations
  ...
  *** Prerequisites
  ...
  Needed:
  - We assume a basic knowledge of Rocq and of defining functions by recursion
...
(** [Equations] is compatible with usual facilities provided by Rocq to write
    terms:
    ...
    - We can use underscores "_" for terms that we do not use, terms
      that Rocq can infer on its own, but also to describe what to do in all
...
(** The first one is usually called the wrapper and the other the worker.
    ...
    Rocq cannot invent the second one on its own.
